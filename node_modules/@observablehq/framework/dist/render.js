...
<meta name="generator" content="Observable Framework v${"1.12.0"}">
...
async function renderModule(root, path, options) {
  const module = findModule(root, path);
  if (!module)
    throw enoent(path);
  const input = `${(await getModuleStaticImports(root, path)).map((i) => `import ${JSON.stringify(i)};
